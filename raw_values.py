...
def get_attachment_strings_to_be_replace(attachment_file_name, instance_xml, attachment_file_extension):
    ...
    if m:
        ...
        # i = 1
        # while True:
        #     matched_file_name = m.group(1)  # use i for more groups #for loop
        #     print(matched_file_name)
        #     if attachment_file_name in matched_file_name:
        #         matched_strings.append(matched_file_name)
        #     i += 1
    return []


def trim_after_hyphen(a):
...
for attachment_key in attachments_keys:
    # attachment_key = attachments_keys[0]
    ...
    # print(attachment_file_name)
    if attachment_file_name in values['instance']:
    #     found attachment file name ie enketo attachment key in instance xml just replace
        file_names_to_be_replaced_in_xml = get_attachment_strings_to_be_replace(attachment_file_name, values['instance'], attachment_file_extension)
        # file_name_to_replace_in_xml = attachment
        if file_names_to_be_replaced_in_xml:
            for file_name_to_be_replaced_in_xml in file_names_to_be_replaced_in_xml:
                print("replacing  ", file_name_to_be_replaced_in_xml, attachment)
                values['instance'] = values['instance'].replace(file_name_to_be_replaced_in_xml, attachment)
    else:
        #     attachment file name have changed trim the kobocat aded text from attachment key
        trimmed_file_name = trim_after_hyphen(attachment_file_name)
        if trimmed_file_name in values['instance']:
            #     found trimmed attachment file name ie enketo attachment key in instance xml just replace
            file_names_to_be_replaced_in_xml = get_attachment_strings_to_be_replace(trimmed_file_name, values['instance'],
