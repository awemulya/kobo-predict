...
def send_message(fxf, status=None, comment=None, comment_url=None):
    roles = UserRole.objects.filter(site=fxf.site, ended_at__isnull=True, group__name="Site Supervisor")
...
def send_message_stages(site):
    roles = UserRole.objects.filter(site=site, ended_at__isnull=True, group__name="Site Supervisor")
    emails = [r.user.email for r in roles]
    Device = get_device_model()
